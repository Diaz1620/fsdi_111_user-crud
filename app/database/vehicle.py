...
def update_vehicle(vehicle_id, license_plate, v_type, color, parking_spot_no, description, user_id):
    cursor = get_db()
    cursor.execute(
        "UPDATE vehicle SET license_plate=? v_type=? color=? parking_spot_no=? description=? user_id=? WHERE id=?", (vehicle_id, license_plate, v_type, color, parking_spot_no, description, user_id)
    )
    cursor.commit()
    cursor.close()
